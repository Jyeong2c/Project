...
/*환자 데이터를 받는 함수*/
void PatientModel::patientData(const QList<QString> &_contactName,          // 환자 성함 column
                                const QList<int> &_age,                     // 환자 나이 column
                                const QList<QString> &_doctorID)            // 담당의사 아이디 column
{
    /*각 환자 데이터 초기화 후 입력*/
    contactName.clear();                // 환자 성함 정보 클리어
    contactName = _contactName;         // 환자 성함 정보 입력
    age.clear();                        // 환자 나이 정보 클리어
    age = _age;                         // 환자 나이 정보 입력
    doctorID.clear();                   // 담당의사 로그인 정보 클리어
    doctorID = _doctorID;               // 담당의사 로그인 정보 입력
    return;
}
