...
{
    manager->deleteLater();                 // Patitent class 종료시 매니저 소멸
    downLoader->deleteLater();              // Patitent class 종료시 다운로드 class 소멸
}

/*해당 환자 이미지를 다운로드 하는 함수*/
    ...
    if (reply->error( ) == QNetworkReply::NoError) {
        ...
        for(int i = 0; i < jsonArr.size(); i++) {
            ...
            if(PatientName == _patient){                          // 선택된 환자의 이름을 찾은 경우
                ...
                connect(downLoader, &Downloader::sendDownload, this, &PatientView::middlePatient);     // 다운로드 신호 중계
