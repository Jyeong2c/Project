...
void KeyFeaturesForm::updateInfo()
{
    ...
    QModelIndex ix= indexes.first();
        ...
        keyModel->setData(ix.siblingAtColumn(25), c25);
        keyModel->setData(ix.siblingAtColumn(26), c26);
        keyModel->setData(ix.siblingAtColumn(27), c27);
...
void KeyFeaturesForm::slot_checkUpdate()
{
    ...
    qDebug() << "처음 실행됩니다." << c1 << "/" <<  c2 << "/" << c3 << "/" << c4 << "/" << c5;
}
