    ...
    , ui(new Ui::MainWindow)
{
    ...
    subWindow->setGeometry(0, 0, 800, 370);      /*매개인자 1) x좌표, 2) y좌표, 3) 위젯의 width, 4) 위젯의 height*/
    ...
    TcpSubWindow[0]->setGeometry(0, 370, 800, 370);/*매개인자 1) x좌표, 2) y좌표, 3) 위젯의 width, 4) 위젯의 height*/
    ...
    this->resize(1500, 800);        /*출력되는 프로젝트의 넓이와 높이를 조젛 width : 1500, height : 840*/
    this->setMaximumWidth(1500);    /*Mdi의 남는 영역이 보이지 않도록 최대 넓이 지정*/
    this->setMaximumHeight(800);    /*Mdi의 남는 영역이 보이지 않도록 최대 높이 지정*/
    ...
    connect(clientmanager, SIGNAL(TCPClientModify(int, QString,int)),
            tcpserver, SLOT(modifyClient(int, QString,int)));   /*고객데이터를 수정하는 커넥트*/
