...
void MainWindow::patientLoad()
{
    ...
    if(DB.open()){                                                 //조건문
        ...
        /*ui 좌측 하단 테이블 뷰에 환자 정보 데이터 베이스 테이블 출력*/
        ui->patientTableView->setModel(patientQueryModel);
        /*6번째 헤더는 테이블에 표시하지 않음*/
        ...
        patientManager = new QNetworkAccessManager(this);
        QObject::connect(patientManager, SIGNAL(finished(QNetworkReply*)),
                         &eventLoop, SLOT(quit()));

        /*요쳥하는 네트워크 경로 지정*/
        QNetworkRequest req( QUrl( QString("http://" + hostName +  ':' + portNum + "/patient/") ) );
        QNetworkReply *reply = patientManager->get(req);
        ...
        if (reply->error( ) == QNetworkReply::NoError) {
            ...
            for(int i = 0; i < jsonArr.size(); i++) {
                ...
                /*의사의 아이디 정보 마다 환자 정보를 다르게 출력할 수 있는 구문*/
                QString DoctorID = "osstem2";
                /*해당하는 의사 아이디에 맞춘 환자 정보를 테이블로 출력*/
...
void MainWindow::onFinished(QNetworkReply* reply)
{
    ...
    if (reply->error( ) == QNetworkReply::NoError) {
        ...
        for(int i = 0; i < jsonArr.size(); i++) {
            ...
            /*이미지 정보를 라인에디트나 다른 전역 변수에 저장하여 이미지의 픽셀당 길이를 곱하여 실길이 측정*/
...
void Downloader::setFile(QString fileURL, QString folderName, QString fileName)
{
    ...
    reply = imageManager->get(request);

    /*서버로부터 받은 데이터를 파일로 전환*/
    file = new QFile;
    file->setFileName(saveFilePath);

    /*요청에 따른 서버 내 이미지 파일 다운로드와 완료여부를 확인하는 connect함수들*/
    connect(reply,SIGNAL(downloadProgress(qint64,qint64)),this,SLOT(onDownloadProgress(qint64,qint64)));
    connect(imageManager,SIGNAL(finished(QNetworkReply*)),this,SLOT(onFinished(QNetworkReply*)));
...
{
    /*읽은 바이트 수 - 처음 대기 상태 바이트 (1) -> 바이트 읽기 완료*/
...
void Downloader::onFinished(QNetworkReply * reply)
{
    ...
    case QNetworkReply::NoError:
    {
        ...
        /*mainwindow의 loadImages함수 호출 시그널을 보냄*/
        emit sendUpload();
        qDebug("[%s] %s : %d", __FILE__, __FUNCTION__, __LINE__);
    }break;
    default:{
        /*그 밖의 에러사항이 있는 경우 에러 메세지 출력*/
